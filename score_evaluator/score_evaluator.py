    ...
    def calculate_weighted_score(self, categories):
        ...
        for category, value in categories.items():
            if category in self._config['scoring_system']['categories']:
                weight = self._config['scoring_system']['categories'][category]['weight']
                score += weight * value
        return score

    def is_spam(self, categories):
        # Check if any category exceeds the single category threshold
        single_category_threshold = self._config['scoring_system']['thresholds']['single_category']
        for category, value in categories.items():
        ...
        overall_threshold = self._config['scoring_system']['thresholds']['overall_score']
